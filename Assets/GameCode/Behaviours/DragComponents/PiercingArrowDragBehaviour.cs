...
public class PiercingArrowDragBehaviour : MonoBehaviour
{
    ...
    void OnEnable()
    {
        ...
        for (int i = 0; i < _heroes.Length; ++i)
        {
            ...
            {
                if (_transforms[i].gameObject.GetComponent<AscaliaBehaviour>())
                    hero = _transforms[i];
            }
        }
        _heroes.Dispose();
    ...
    void Update()
    {
        if (hero != null)
        {
            var ascalia = hero.GetComponent<AscaliaBehaviour>();
            if (ascalia != null)
            {
